...
func (b *Cowbuilder) cowbuilderCommand(d deb.Codename, a deb.Architecture, deps []*AptRepositoryAccess, command string, args ...string) (*exec.Cmd, error) {
	...
	fmt.Fprintf(f, "%s=\"%s\"\n", "EXTRAPACKAGES", strings.Join(extrapackages, " "))
...
# File automatically generated by ddesk, do not edit
#
...
`, dep)
		...
		//dist does not necersarly ships apt-transport-https package, and
		//EXTRAPACKAGES install comes after

		if strings.HasPrefix(dep.Address, "https://") == true &&
			aptHttpsSupportAdded == false {
			fmt.Fprintf(&content, "apt-get install --yes apt-transport-https ca-certificates\n")
			aptHttpsSupportAdded = true
		}
